...
export async function generateGarenaAccount(region, prefix = 'Yax') {
  ...
  try {
    ...
    log('STEP_1', `resp: ${regText.slice(0, 300)}`);
    ...
    try {
      ...
      log('STEP_1', `FAILED - not JSON`);
      return null;
    }

    if (regJson.code !== 0) {
      log('STEP_1', `FAILED - code=${regJson.code}`);
      return null;
    }
    ...
    log('STEP_2', `resp: ${tokText.slice(0, 300)}`);
    ...
    try {
      ...
      log('STEP_2', `FAILED - not JSON`);
      return null;
    }

    if (tokJson.code !== 0) {
      log('STEP_2', `FAILED - code=${tokJson.code}`);
      return null;
    }

    const accessToken = tokJson.data.access_token;
    const openId = tokJson.data.open_id;
    log('STEP_2', `SUCCESS | token_len=${accessToken.length}`);
